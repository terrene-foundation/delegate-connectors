...
def build_action_signing_bytes(
    ...
    *,
    ...
    Signs over ``{payload, signer_delegate_id, action_id, observed_at}`` (not
    the bare ``payload``), so two writes with an identical payload produce
    DIFFERENT signed bytes (distinct ``action_id`` + ``observed_at``) and
    the signer / action id / observation time are cryptographically bound —
    closing the replay/forge surface where same-payload receipts would
...
def build_read_signing_bytes(
    ...
    *,
    ...
    Signs over ``{manifest, attester_delegate_id, read_id, observed_at}`` —
    the attester / read id / observation time are bound into the attestation.
    """
    return canonical_json_dumps(
        {
            "manifest": manifest,
...
def verify_action_envelope(
    ...
    *,
    ...
    fields (``payload`` + ``signer_delegate_id`` + ``action_id`` + the
    supplied ``observed_at``) and checks (a) the re-derived bytes equal the
    signed ``envelope.canonical_bytes`` AND (b) the Ed25519 signature
    verifies. Tamper with ``signer_delegate_id`` / ``action_id`` / ``payload``
    and the re-derived bytes diverge from the signed bytes, so verification
    fails.
    """

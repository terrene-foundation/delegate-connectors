...
KNOWN SDK BLOCKER — ``runtime.execute()`` (kailash-py#1182):
    ...
    ``write`` receipts verify correctly (proven in the Tier-1 suite). The
    end-to-end ``execute()`` assertion is gated on the SDK fix; composition
    (everything this module does up to :func:`build_whatsapp_runtime`) is not.
"""

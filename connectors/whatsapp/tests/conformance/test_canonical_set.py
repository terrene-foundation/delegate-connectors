...
1. **Now (this commit):** the vendored canonical set loads, every vector is
   ...
   without raising. These are concrete, run today.
...
@pytest.mark.xfail(
    reason=(
        ...
        "compose.py § KNOWN SDK BLOCKER + specs/conformance.md § When unblocked."
    ),
    strict=True,
)
...
@pytest.mark.xfail(
    reason=(
        ...
        "and assert_receipts_agree across the two deterministic runs. See "

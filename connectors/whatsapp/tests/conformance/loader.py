...
This is a near-exact mirror of the email connector's
``connectors/email/tests/conformance/loader.py`` — the WhatsApp loader lives at
the SAME directory depth (``connectors/whatsapp/tests/conformance/loader.py``),
so the parent-ascent count to the monorepo root is identical (four parents).
...
at ref ``main``; the WhatsApp harness REUSES the monorepo-shared copy (no
per-connector fork).
"""
...
def _default_fixture_path() -> Path:
    ...
    `connectors/whatsapp/tests/conformance/loader.py` → ascend four parents to
    ...
    # loader.py -> conformance -> tests -> whatsapp -> connectors -> repo root
...
class VendoredConformanceLoader:
    ...
    def load(self) -> list[ConformanceVector]:
        ...
        the shipped loader would raise — but pointed at the shared vendored
        path).
        """
        if not self._path.exists():
            raise FileNotFoundError(
                f"vendored canonical conformance fixture not found at {self._path}; "
                "the WhatsApp harness reuses the monorepo-shared fixture at "

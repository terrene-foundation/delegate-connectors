...
1. **Well-formedness:** the vendored canonical set loads, every vector is
   ...
   against ``DelegateRuntime`` without raising.
...
2. **Per-vector outcome (ACTIVE on kailash >= 2.28.0):** the per-vector driver
   ...
   connector-agnostic (they exercise the delegate spine, not the email send
   Accept-path): DV-7/DV-9 drive ``runtime.execute()`` against an SMTP transport
   with no local server, so both deterministic runs terminate as
   ``phase=="failed"`` and their receipts agree; DV-9 still Accepts because the
   audit chain populates from the pre-dispatch phase transitions (the §9
   round-trip is over the audit entries), and DV-7 Rejects because the runtime is
   terminal. (The Tier-2 Mailpit/GreenMail e2e Accept-path is a separate surface.)
"""
...
def _make_composed():
    """Fresh composed Email runtime for the per-vector driver.

    Each call builds an independent ``ComposedEmailRuntime``; the driver needs
    fresh runtimes (DV-7 is single-shot, DV-9 needs a freshly-run audit engine,
    the determinism rows need two independent runs). The SMTP/IMAP transports
    point at localhost ports with no server running in the conformance tier — the
    driver exercises the delegate spine, and a dispatch that reaches the absent
    SMTP server simply terminates the run as ``phase=="failed"``, which the §7/§9
    vectors handle identically. (The Tier-2 Mailpit/GreenMail e2e Accept-path is a
    separate surface.)
    """
    smtp = SmtpTransport(SmtpConfig(host="localhost", port=1025))
    imap = ImapTransport(ImapConfig(host="localhost", port=3143))
...
async def test_assert_receipts_agree_across_deterministic_runs() -> None:
    ...
    deep-compares the two ``RuntimeExecutionResult`` receipt trees minus the
    per-run-by-design fields (``run_id`` + the per-transition ``at`` timestamp,
    plus ``dispatch_id`` / ``audit_head_hash`` / ``audit_chain_entries`` which
    incorporate the fresh dispatch UUID and per-run audit state) — the SAME
    ``exclude_fields`` as the Tier-2 e2e determinism test. Both runs terminate
    identically (the SMTP server is absent in the conformance tier), so their
    receipts agree.
    """

...
This is a near-exact mirror of the email + WhatsApp connectors' ``loader.py`` —
the Slack loader lives at the SAME directory depth
(``connectors/slack/tests/conformance/loader.py``), so the parent-ascent count to
the monorepo root is identical (four parents). The ascent
``Path(__file__).resolve().parents[4]`` lands on the repo root (verified to
resolve to the ``delegate-connectors`` checkout).
...
at ref ``main``; the Slack harness REUSES the monorepo-shared copy (no
per-connector fork) — it does NOT re-source from kailash-py.
"""
...
def _default_fixture_path() -> Path:
    ...
    `connectors/slack/tests/conformance/loader.py` → ascend four parents to the
    monorepo root, then descend into the shared fixture directory. The
    resolution is path-based (not import-based) so it survives editable installs
    that place the source tree at an arbitrary checkout location.
    """
    here = Path(__file__).resolve()
    # loader.py -> conformance -> tests -> slack -> connectors -> repo root
...
class VendoredConformanceLoader:
    ...
    def load(self) -> list[ConformanceVector]:
        ...
        Raises ``FileNotFoundError`` if the fixture is absent (the same shape the
        shipped loader would raise — but pointed at the shared vendored path).
        """
        if not self._path.exists():
            raise FileNotFoundError(
                f"vendored canonical conformance fixture not found at {self._path}; "
                "the Slack harness reuses the monorepo-shared fixture at "

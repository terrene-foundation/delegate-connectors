...
1. **Now (this commit):** the vendored canonical set loads, every vector is
   ...
   ``build_telegram_runtime``) without raising. These are concrete, run today.
...
2. **Gated on kailash-py#1182 (SDK ``runtime.execute()`` audit-signature fix):**
   ...
   (the audit-emit path signs the event PAYLOAD bytes while
...
def test_vector_outcome_matches_expected(vector: ConformanceVector) -> None:
    ...
    per-vector scenario setup is deferred to the un-xfail shard so it lands
    against a working ``execute()`` rather than a known-failing one. The
    strict-xfail is the contract that this test eventually flips to a real
    assertion.
    """
...
@pytest.mark.xfail(
    reason=(
        "SDK bug kailash-py#1182: deterministic-run receipt agreement cannot be "
        "measured while runtime.execute() returns phase=='failed' under any real "
        "verifier (audit-emit signs payload bytes; AuditChainEngine verifies the "
        "full entry signing bytes — compose.py § KNOWN SDK BLOCKER). When #1182 "
        "ships, this strict xfail flips to XPASS and the marker MUST be removed + "
        "the body wired to run the full vector set TWICE through composed runtimes "
        "and assert_receipts_agree across the two deterministic runs."
    ),
    strict=True,
)
def test_assert_receipts_agree_across_deterministic_runs() -> None:
    ...
    The strict-xfail is the contract that this flips to a real assertion when
    the SDK fix lands.
    """

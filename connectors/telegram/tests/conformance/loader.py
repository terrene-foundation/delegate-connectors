...
``kailash-py`` SOURCE repo only and is not shipped in the PyPI wheel. This
module loads the vendored copy at
...
This is a near-exact mirror of the WhatsApp / email connector loaders — the
Telegram loader lives at the SAME directory depth
(``connectors/telegram/tests/conformance/loader.py``), so the parent-ascent
count to the monorepo root is identical (four parents). The harness REUSES the
monorepo-shared copy (no per-connector fork; no re-source from kailash-py).
"""
...
def _default_fixture_path() -> Path:
    ...
    ``connectors/telegram/tests/conformance/loader.py`` → ascend four parents to
    the monorepo root, then descend into the shared fixture directory. The
    ...
    # loader.py -> conformance -> tests -> telegram -> connectors -> repo root
    ...
    def load(self) -> list[ConformanceVector]:
        ...
        if not self._path.exists():
            raise FileNotFoundError(
                ...
                "the Telegram harness reuses the monorepo-shared fixture at "

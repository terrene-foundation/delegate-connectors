...
def build_action_signing_bytes(
    ...
    *,
    ...
    DIFFERENT signed bytes (distinct ``action_id`` + ``observed_at``) and the
    signer / action id / observation time are cryptographically bound —
...
def build_read_signing_bytes(
    ...
    *,
    ...
    Signs over ``{manifest, attester_delegate_id, read_id, observed_at}`` (not
    the bare ``manifest``), so the attester / read id / observation time are
    bound into the attestation.
    """
    return canonical_json_dumps(
        {
            "manifest": manifest,
...
def verify_action_envelope(
    ...
    *,
    ...
    ``envelope.canonical_bytes`` AND (b) the Ed25519 signature verifies.
    Tamper with ``signer_delegate_id`` / ``action_id`` / ``payload`` and the
    re-derived bytes diverge from the signed bytes, so verification fails.
    """
